...
- **组件规范**：Activity 必须 `@Route(path = RouterPath.Xxx.Yyy)`；资源前缀、包名地图与源码分包见 [docs/01-rules/structure.md](docs/01-rules/structure.md)（兄弟横评模块用能力包→activity/data；控件集合模块用模块级 activity/）；modules 模块依赖 `basic_lib` 与 `basic_shared`。
