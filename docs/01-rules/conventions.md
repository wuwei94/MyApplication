...
- **媒体域聚合（「多媒体」一级分类）**：`module_image_loader`（图片加载管道）、`module_media`（相机采集）、`module_gpuimage`（图像处理引擎）同属媒体域，统一挂「多媒体」分类，按「图片加载 / 相机采集 / 图像处理」分组展示。三者分别是资源库、硬件采集与处理引擎能力，既不是 UI 控件、也不属「系统能力」——媒体域内容不应拆分到其他一级分类。
