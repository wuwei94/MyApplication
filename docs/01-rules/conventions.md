...
- **二级按技术来源或主题**：同一能力的不同实现按来源分组（控件分「官方控件 / 自定义控件 / 第三方 UI 库」），不同技术点按主题分组（UI 交互分「控件 / 导航 / 动画 / 内容渲染 / 数据可视化」，网络分「HTTP / 流式推送 / Socket / 消息队列 / 蓝牙通信」，多媒体分「图片加载 / 相机采集 / 图像处理」，系统能力分「系统原生 / 安全 / 跨进程通信」）；分组标题以「便于横向对比、快速定位」为准。分组承载**技术主题**、入口承载**模块**，两者粒度不同，因此**分组下只有 1 个模块属正常粒度**（网络通信的「蓝牙通信」、系统能力的「安全」「跨进程通信」都是），并作为该主题的扩展位，不因条目少而拍平。
- **媒体域聚合（「多媒体」一级分类）**：`module_image_loader`（图片加载管道）、`module_media`（相机采集）、`module_graphics`（原生图像与图形渲染计算）与 `module_gpuimage`（图像滤镜引擎）同属媒体域，统一挂「多媒体」分类，按「图片加载 / 相机采集 / 图像处理」分组展示。它们是资源库、硬件采集与图像渲染处理引擎能力，既不是 UI 控件、也不属「系统能力」——媒体域内容不应拆分到其他一级分类。
...
- **底层能力 vs 第三方 UI 控件**：
  - `module_graphics`：聚焦系统原生底层图像与图形渲染计算、着色器与渲染特效（RenderEffect / RenderScript 模糊与离屏渲染）；与第三方基于 OpenGL ES 的滤镜库 `module_gpuimage` 形成「原生 vs 第三方」的技术互补。
