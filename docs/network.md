...
本文档集中说明 `lib_okhttp`、`lib_retrofit`、`lib_retrofit_rx` 和 `lib_ktor` 的当前职责、使用入口、生命周期和功能边界。这里只描述当前代码，不记录历史重构过程。

## 依赖关系与选型

```text
lib_retrofit_rx -> lib_retrofit -> lib_okhttp -> OkHttp
...
| `lib_okhttp + lib_retrofit + lib_retrofit_rx` | Retrofit 注解接口，特殊场景支持动态请求 | RxJava3 `Single` | 需要维护既有 RxJava 调用链的项目 |
...
- HTTP 非 2xx、连接、超时、SSL 和解析错误转换为简单的 `ApiException(code/message/cause)`。
...
api.login(username, password)
    ...
    .subscribe(object : RxResponseCallback<LoginData>() {
...
`RxDynamicRequest` 只用于 URL、HTTP 方法或请求结构必须在运行时确定的场景。固定业务 API 应继续使用 Retrofit 注解接口。动态 Builder 支持 GET/POST/PUT/PATCH/DELETE、Form、JSON、Raw Body 和 Multipart，并在执行前生成不可变配置快照。

### 当前限制

`withNetworkDefaults()` 和 `RxDynamicRequest.createResponse()` 当前没有统一应用 `RxServerResultFunction`，而 `RxResponseCallback.onSuccess()` 也不检查业务码。因此非零业务码可能进入成功回调；部分调用点手工添加 `RxServerResultFunction` 后又会进入失败回调。

在统一契约前，调用方必须明确选择一种方式，不能假设默认链会自动把业务失败转为 `ApiException`。
