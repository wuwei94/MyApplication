...
|------|------|------|--------------|---------|
| UI 交互 | module_widget | 标准控件 | WidgetMainActivity | /Widget |
| UI 交互 | module_widget_custom | 自定义控件 | WidgetCustomMainActivity | /WidgetCustom |
| UI 交互 | module_widget_thirdparty | 第三方 UI 库 | WidgetThirdpartyMainActivity | /WidgetThirdparty |
| UI 交互 | module_chart | 数据可视化图表（MPAndroidChart） | ChartMainActivity | /Chart |
| UI 交互 | module_tab | Tab 导航 | TabMainActivity | /Tab |
...
| 网络通信 | module_mqtt | MQTT 发布 / 订阅 | MqttMainActivity | /Mqtt |
| 网络通信 | module_bluetooth | 蓝牙通信（BLE 客户端） | BluetoothMainActivity | /Bluetooth |
| 数据存储 | module_database | 数据库 | DatabaseMainActivity | /Database |
...
| 架构与工程 | module_arch | 架构模式 | ArchMainActivity | /Arch |
| 架构与工程 | module_di | 依赖注入 | DiMainActivity | /Di |
| 架构与工程 | module_event | 事件总线 | EventMainActivity | /Event |
| 架构与工程 | module_reactive | 响应式编程 | ReactiveMainActivity | /Reactive |
| 架构与工程 | module_performance | 性能优化 | PerformanceMainActivity | /Performance |
| Kotlin & Jetpack | module_kotlin | Kotlin 特性 | KotlinMainActivity | /Kotlin |
| Kotlin & Jetpack | module_jetpack | Jetpack 组件 | JetpackMainActivity | /Jetpack |
| Compose & Flutter | module_compose | Compose UI | ComposeMainActivity | /Compose |
| Compose & Flutter | module_flutter | Flutter 集成 | FlutterMainActivity | /Flutter |
| Sample & Feature | module_sample | 技术示例 | SampleMainActivity | /Sample |
| Sample & Feature | module_feature | 业务功能 | FeatureMainActivity | /Feature |
...
> 第三方图表库 MPAndroidChart 已独立为 `module_chart`（数据可视化），不再属于本模块。
...
### module_chart（数据可视化图表）

演示 Android 生态最主流的原生图表库 MPAndroidChart。以 `Chart` 基类衍生出的图表视图（LineChart / BarChart / PieChart / RadarChart）配合 `DataSet` 数据模型、`MarkerView` 悬浮提示与 `Highlight` 高亮机制，完成图表渲染与用户交互。

| Activity | 功能 |
|----------|------|
| ChartMainActivity | 模块入口，导航到基础图表与全景联动看板示例页面 |
...
> 同类主题的 Compose Canvas 自绘图表见 `module_compose` 的 LineChart / BarChart / PieChart / RadarChart / ChartLinkage 五个页面。
...
| OfflineFirstActivity | Offline-First 离线优先与 SSOT 单一数据源架构模式（Room 实体 SSOT + Proto DataStore 版本游标 + NetworkMonitor + SyncWorker；AppInit 启动时 Sync.initialize 排队） |
