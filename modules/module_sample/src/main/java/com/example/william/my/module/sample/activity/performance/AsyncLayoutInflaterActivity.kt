...
import com.example.william.my.module.sample.preload.ViewPreloadManager
 ...
 */
@Route(path = RouterPath.Sample.AsyncLayoutInflater)
class AsyncLayoutInflaterActivity : BasicLayoutActivity() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        // 配置详情页布局的最大预加载容量
        ViewPreloadManager.setMaxCount(R.layout.sample_layout_async_detail, 3)
        ViewPreloadManager.setMaxCount(R.layout.sample_layout_async_card, 5)
    ...
    private fun testAsyncInflateCard() {
        ...
        asyncInflater.inflate(R.layout.sample_layout_async_card, mContainer) { view, _, _ ->
            val cost = System.currentTimeMillis() - startTime
            val titleView = view.findViewById<TextView>(R.id.sample_async_card_title)
            val descView = view.findViewById<TextView>(R.id.sample_async_card_desc)
    ...
    private fun testPreloadDetailViews() {
        ...
        ViewPreloadManager.preload(this, R.layout.sample_layout_async_detail, count = 2) { loadedCount ->
            val cost = System.currentTimeMillis() - startTime
            val currentPoolSize = ViewPreloadManager.getPoolSize(R.layout.sample_layout_async_detail)
    ...
    private fun testConsumeDetailView() {
        val hasCache = ViewPreloadManager.hasCache(R.layout.sample_layout_async_detail)
        val startTime = System.currentTimeMillis()

        // 核心 API：无论池中有无，直接调用 getView()
        val detailView = ViewPreloadManager.getView(this, R.layout.sample_layout_async_detail)
        val cost = System.currentTimeMillis() - startTime

        val tagView = detailView.findViewById<TextView>(R.id.sample_detail_tag)
        val footerView = detailView.findViewById<TextView>(R.id.sample_detail_footer)

        if (hasCache) {
            tagView.text = "⚡ [0ms 命中预加载池] 详情页 View 瞬间取出并挂载！"
            footerView.text = "状态：命中 ViewPreloadManager 预加载池（获取耗时：${cost}ms，剩余池容量：${ViewPreloadManager.getPoolSize(R.layout.sample_layout_async_detail)}）"
        } else {
    ...
    private fun testPreloadAndShowBottomSheet() {
        // 预加载一个卡片布局作为弹窗内容
        ViewPreloadManager.preload(this, R.layout.sample_layout_async_card, count = 1) {
            val dialogView = ViewPreloadManager.getView(this, R.layout.sample_layout_async_card)
            val titleView = dialogView.findViewById<TextView>(R.id.sample_async_card_title)
            val descView = dialogView.findViewById<TextView>(R.id.sample_async_card_desc)
    ...
    private fun showPreloadManagerStatus() {
        val detailPoolSize = ViewPreloadManager.getPoolSize(R.layout.sample_layout_async_detail)
        val cardPoolSize = ViewPreloadManager.getPoolSize(R.layout.sample_layout_async_card)

        showHintView(
            "📊 【ViewPreloadManager 缓存池运行状态】\n\n" +
                    "• 详情页布局（sample_layout_async_detail）当前就绪池大小：$detailPoolSize / 上限: 3\n" +
                    "• 卡片/弹窗布局（sample_layout_async_card）当前就绪池大小：$cardPoolSize / 上限: 5\n\n" +
