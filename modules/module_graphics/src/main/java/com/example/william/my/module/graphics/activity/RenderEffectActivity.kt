...
/**
 * RenderEffect — Android 12+ 推荐模糊方案
 *
 * Android 12（API 31）引入 RenderEffect，是 RenderScript 的官方替代方案。
 * 本示例演示两种模糊实现方式及其适用场景。
 *
 * 核心机制与避坑点：
 * 1. View.setRenderEffect()：直接对 View 应用 GPU 加速模糊，代码最简；内部由 RenderThread 异步执行，不阻塞 UI 线程；模糊结果直接渲染到 View 层级，无法导出为 Bitmap
 * 2. HardwareRenderer 离屏渲染：将 Bitmap 渲染到 RenderNode，应用模糊后输出为新 Bitmap；使用 ImageReader + HardwareBuffer 获取 GPU 渲染结果；适合需要保存 / 分享模糊图的场景
 * 3. 模糊半径可调：radiusX / radiusY 范围 0.1 ~ 25.0
 * 4. 边缘模式：TileMode.CLAMP（边缘自然过渡，推荐）/ REPEAT / MIRROR
 *
 * @see RenderScriptActivity RenderScript 模糊方案（已废弃）
 * https://developer.android.com/develop/ui/views/graphics/rendereffects
 */
...
class RenderEffectActivity : BasicImageActivity() {
    ...
    private fun applyViewBlur() {
        binding.basicsImage.setRenderEffect(
    ...
    private fun clearBlur() {
        binding.basicsImage.setRenderEffect(null)
        showImage(originalBitmap)
        isBlurred = false
        currentMethod = ""
    }
