 ...
 *
 * 核心机制与避坑点：
 * 1. 作用域函数：with / let / run / also / apply，区分接收者（this 或 it）与返回值（Lambda 结果或对象本身）
 * 2. 泛型实化：inline + reified T 绕过 JVM 泛型擦除，直接获取 T::class.java
 * 3. 自定义内联扩展：高阶函数内联消除 Lambda 对象分配开销
 * 4. 内联修饰符：noinline 与 crossinline 控制 Lambda 是否内联及非局部返回行为
 *
