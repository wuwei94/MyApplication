 ...
 *
 * 核心机制与避坑点：
 * 1. 类委托：通过 by 关键字将接口实现委托给内部实例，装饰模式语法糖
 * 2. 属性委托：通过 ReadOnlyProperty 或 ReadWriteProperty 拦截属性的 getValue / setValue
 * 3. 延迟初始化：by lazy 延迟求值（线程安全默认使用 LazyThreadSafetyMode.SYNCHRONIZED）
 * 4. 属性监听与否决：Delegates.observable 监听变更并接收新旧值；Delegates.vetoable 条件拦截决定是否接受新值
 * 5. Map 映射委托：通过 val property: Type by map 将属性名作为 Key 直接从 Map 读取
 *
