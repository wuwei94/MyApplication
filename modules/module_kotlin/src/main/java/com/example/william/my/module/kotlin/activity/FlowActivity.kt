...
/**
 * Kotlin Flow — 响应式冷热数据流
 *
 * Flow 是 Kotlin 协程的响应式数据流抽象，用于处理异步数据序列。
 * 本页演示冷流收集、数据变换、双流组合、防抖搜索、热流对比与异常重试六大核心能力。
 *
 * 核心机制与避坑点：
 * 1. 冷流机制：仅在 collect 时触发生产，未收集不执行，资源零浪费
 * 2. 变换操作符：map / filter / take 惰性数据流变换，支持链式组合
 * 3. 组合操作符：zip（严格对齐配对）与 combine（最新值组合）处理多流场景
 * 4. 防抖搜索：debounce + distinctUntilChanged + flatMapLatest 优雅处理输入检索
 * 5. 热流架构：StateFlow（状态持有，replay=1）与 SharedFlow（事件广播，无 replay）
 *
...
class FlowActivity : BasicResponseActivity() {
    ...
    override fun onRecyclerClick(position: Int, string: String) {
        ...
        when (position) {
            ...
            1 -> viewModel.testFlowTransform()
            2 -> viewModel.testZipAndCombine()
            3 -> viewModel.testDebounceAndFlatMapLatest()
            4 -> viewModel.testHotFlows()
            5 -> viewModel.testRetryAndCatch()
        }
    }
