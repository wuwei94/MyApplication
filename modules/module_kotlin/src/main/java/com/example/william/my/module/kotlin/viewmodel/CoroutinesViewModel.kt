...
import com.example.william.my.module.kotlin.usecase.LoginCoroutinesUseCase
 ...
 */
class CoroutinesViewModel(private val useCase: LoginCoroutinesUseCase) : ViewModel() {
        ...
        viewModelScope.launch {
            ...
            val result: NetworkResult<RetrofitResponse<LoginData>> =
                try {
                    ...
                    NetworkResult.Error(Exception("网络请求失败"))
                }

            when (result) {
                is NetworkResult.Success<RetrofitResponse<LoginData>> -> {
                    _coroutineLog.postValue("✓ [1. 基础请求] ${Gson().toJson(result.data)}")
                }

                is NetworkResult.Error -> {
                    _coroutineLog.postValue("✗ [1. 基础请求] ${result.exception.message}")
                }
    ...
    companion object {
        /**
         * 工厂：通过 [viewModelFactory] DSL 构建并注入 [LoginCoroutinesUseCase]
         */
        val Factory: ViewModelProvider.Factory = viewModelFactory {
            initializer {
                CoroutinesViewModel(LoginCoroutinesUseCase(Dispatchers.IO))
            }
        }
    }
}
