...
class PagingActivity : BasicRecyclerActivity() {
    ...
    private fun initPagingAdapter() {
        adapter = PagingAdapter(PagingAdapter.PagingComparator())
        ...
        dataRecycler.adapter = adapter.withLoadStateHeaderAndFooter(
    ...
    private fun switchMode(mode: PagingMode) {
        ...
        when (mode) {
            ...
            PagingMode.MEDIATOR_RX -> {
                rxDisposable = viewModel.articlesByMediatorFlowable
            ...
            PagingMode.NETWORK_RX -> {
                rxDisposable = viewModel.articlesByNetworkFlowable
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe { pagingData ->
                        adapter.submitData(lifecycle, pagingData)
