package com.example.william.my.module.bluetooth.rx_ble.activity
 ...
 *
 * 用 Rx 操作符编排连接 → MTU → Notify → 写入；dispose 一次完成断开与注销订阅，与原生 / Nordic / FastBle 连接栈平行对照。
 *
 * 核心机制与避坑点：
 * 1. 连接流：`establishConnection` 产出 `Observable<RxBleConnection>`，参数控制 autoConnect
 * 2. 操作符编排：flatMap / flatMapSingle 串联 discoverServices、requestMtu、read/write
 * 3. 通知流：setupNotification 返回 Observable，需 flatMap 展开 inner Observable 才能收数据
 * 4. 资源释放：Disposable / CompositeDisposable dispose 时自动断开并注销通知，页面销毁前必须执行
 * 5. 线程模型：链路在后台线程执行，UI 更新需切到 AndroidSchedulers.mainThread
 *
 * 官方参考：
...
class BleRxConnectActivity : BasicResponseActivity() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        showDescription(
            "RxAndroidBle 响应式连接与流控示例\n\n" +
                "覆盖连接状态、MTU、读写 Notify、大包写入与 dispose 释放\n" +
                "请按顺序点击下方操作项",
        )
    }
    ...
    override fun buildList(): ArrayList<String> = arrayListOf(
        ...
        "6. 发送大包数据 (writeCharacteristic 分段确认)",
        "7. 断开连接并释放所有流 (dispose)",
    )
    ...
    /**
     * 写入超过单包有效载荷的数据，观察 writeCharacteristic 返回字节数。
     */
    private fun writeLargePayload() {
        ...
        appendLog("→ [大包写入] 发送 ${sendBytes.size} 字节...")
        val d = connObs
            .firstOrError()
            .flatMap { connection -> connection.writeCharacteristic(charUuid, sendBytes) }
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(
                { bytes ->
                    appendLog("✓ [大包写入] 底层确认 ${bytes.size} 字节")
                },
                { e -> appendLog("✗ [大包写入] ${e.message}") },
            )
        disposables.add(d)
    }
