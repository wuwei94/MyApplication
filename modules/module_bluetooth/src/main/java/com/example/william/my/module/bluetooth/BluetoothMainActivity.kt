...
class BluetoothMainActivity : RouterRecyclerActivity() {
    ...
    override fun buildRouter(): ArrayList<RouterItem> {
        ...
        routerItems.add(RouterItem("Nordic 扫描过滤对照（系统 Scanner）", RouterPath.Bluetooth.NordicScan))
        routerItems.add(RouterItem("Nordic BLE 连接与挂起调用", RouterPath.Bluetooth.NordicConnect))
        routerItems.add(RouterItem("Nordic BLE 传输原理示意（split/merge）", RouterPath.Bluetooth.NordicTransfer))
        routerItems.add(RouterItem("", ""))
