...
import 'package:flutter_demo/catalog/models/resolved_catalog_entry.dart';
...
class CatalogRouteFactory {
  ...
  static List<go_router.GoRoute> toGoRoutes(List<CatalogEntry> routeInfos) {
    return _flattenItems(
      CatalogTreeResolver.resolve(routeInfos),
    ).map(_toGoRoute).toList(growable: false);
  }

  /// 将 `List<CatalogEntry>` 转换为 `List<AutoRoute>`
  static List<auto_route.AutoRoute> toAutoRoutes(
    List<CatalogEntry> routeInfos,
  ) {
    return _flattenItems(
      CatalogTreeResolver.resolve(routeInfos),
    ).map(_toAutoRoute).toList(growable: false);
  }
  ...
  static Widget _buildPage(
    ...
    ResolvedCatalogEntry routeItem,
  ) {
    if (routeItem.children.isNotEmpty) {
      return CatalogPage(title: routeItem.title, routes: routeItem.children);
    }

    return routeItem.pageBuilder(context);
  }

  static List<ResolvedCatalogEntry> _flattenItems(
    List<ResolvedCatalogEntry> items,
  ) {
