...
echo ""
echo "卸载：rm $(git rev-parse --git-path hooks)/pre-push $(git rev-parse --git-path hooks)/commit-msg"
echo "单次跳过：git push --no-verify / git commit --no-verify（或 COMMIT_MSG_DISABLE=1）"
