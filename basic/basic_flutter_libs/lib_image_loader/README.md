...
- `IImageLoader` 定义统一接口（`load` / `radius` / `round` / `provider` / `clear`），当前内核为 `CachedNetworkImageLoader`（基于 cached_network_image），负责常规网络图加载、缓存、占位图与错误态。
...
- 大图查看（手势缩放、平移预览）属于查看器场景，与常规加载职责不同，不纳入本包范围，由业务侧直接使用 extended_image 实现。
