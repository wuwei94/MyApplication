...
- 这个项目的导航链路是固定三层：
  ...
  - `FeaturesListPage` 用 `context.push(item.path)` 进入最终示例页
  - 最终示例页由 `GoRouter` 路由表提供
- 新增示例时不要随意改掉这三层结构，也不要把首页分组入口和最终示例页混成同一级。

### 10. 新增页面时同步文件
- 如果只是向现有分组新增页面，通常只需要更新对应的 `lib/navigation/modules/*.dart`。
- 如果新增的是首页一级分组，还要同步更新：
  - `lib/navigation/modules/index.dart`
  - `lib/navigation/app_router.dart`
  - `lib/navigation/registry/route_registry.dart`
