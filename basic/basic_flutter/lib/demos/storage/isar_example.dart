...
import 'package:basic_flutter/demos/storage/isar/isar_task_record.dart';
import 'package:basic_flutter/demos/storage/isar/isar_task_store.dart';
import 'package:flutter/material.dart';
import 'package:timeago/timeago.dart' as timeago;

/// Isar
/// https://pub.dev/packages/isar_community
class IsarDemoPage extends StatelessWidget {
  const IsarDemoPage({super.key, required this.title});

  final String title;

  @override
  Widget build(BuildContext context) {
    return IsarDemoView(title: title);
  }
}

class IsarDemoView extends StatefulWidget {
  const IsarDemoView({super.key, required this.title});
...
class _IsarDemoViewState extends State<IsarDemoView> {
  ...
  late final Future<IsarTaskStore> _storeFuture;

  _IsarTaskFilter _selectedFilter = _IsarTaskFilter.all;
  int _selectedPriority = 2;
  bool _isMutating = false;
  String _statusMessage = '正在准备 Isar Database，稍后会自动加载演示数据。';
  ...
  Future<IsarTaskStore> _prepareStore() async {
    final IsarTaskStore store = await IsarTaskStore.create();
    if (!mounted) {
      return store;
    }

    setState(() {
      _statusMessage = 'Isar 已就绪，当前示例通过 watchLazy() 自动监听对象变化。';
    });
    return store;
  }
  ...
  Future<void> _runMutation({
    ...
    required Future<void> Function(IsarTaskStore store) mutation,
  }) async {
    if (_isMutating) {
      return;
    }
    ...
    try {
      final IsarTaskStore store = await _storeFuture;
      ...
      setState(() {
        _statusMessage = 'Isar 操作失败：$error';
      });
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _createTask() async {
    ...
    if (title.isEmpty) {
      setState(() {
        _statusMessage = '先输入一个标题，再把对象写入 Isar。';
      });
      return;
    }
    ...
    await _runMutation(
      ...
      mutation: (IsarTaskStore store) {
        return store.addTask(
          title: title,
          detail: detail.isEmpty ? '这是一条刚写入 Isar 的示例记录。' : detail,
  ...
  Future<void> _resetDemoData() async {
    await _runMutation(
      ...
      mutation: (IsarTaskStore store) {
        return store.resetWithDemoData();
      },
    );
  }
  ...
  Future<void> _toggleDone(IsarTaskRecord task) async {
    final bool nextState = !task.isDone;
    await _runMutation(
      successMessage: nextState ? '任务已标记完成。' : '任务已恢复为待处理。',
      mutation: (IsarTaskStore store) {
        return store.toggleDone(task);
      },
    );
  }

  Future<void> _toggleStarred(IsarTaskRecord task) async {
    final bool nextState = !task.isStarred;
    await _runMutation(
      successMessage: nextState ? '这条任务已加入星标。' : '这条任务已取消星标。',
      mutation: (IsarTaskStore store) {
        return store.toggleStarred(task);
      },
    );
  }

  Future<void> _deleteTask(IsarTaskRecord task) async {
    await _runMutation(
  ...
  List<IsarTaskRecord> _applyFilter(List<IsarTaskRecord> tasks) {
    return tasks
        .where((IsarTaskRecord task) => _selectedFilter.matches(task))
        .toList();
  }
  ...
  Widget _buildComposerSection() {
    return _SectionCard(
      title: '对象写入',
      subtitle: '输入标题和描述后直接写入 Isar，下面的列表会通过 watchLazy() 自动刷新。',
      child: Column(
  ...
  Widget _buildFilterSection({
    ...
    return _SectionCard(
      ...
      child: Wrap(
        spacing: 8,
        runSpacing: 8,
        children: _IsarTaskFilter.values
            .map(
              (_IsarTaskFilter filter) => ChoiceChip(
  ...
  Widget _buildTaskListSection(List<IsarTaskRecord> tasks) {
    if (tasks.isEmpty) {
      return _SectionCard(
        title: '对象列表',
        ...
        child: _buildCenteredState(
          ...
          title: '列表为空',
          description: 'Isar Database 已准备好，现在可以体验新增、更新和删除操作。',
        ),
      );
    }
...
enum _IsarTaskFilter {
  all('全部'),
  pending('待处理'),
  completed('已完成'),
  starred('星标');

  const _IsarTaskFilter(this.label);

  final String label;

  bool matches(IsarTaskRecord task) {
    switch (this) {
      case _IsarTaskFilter.all:
        return true;
      case _IsarTaskFilter.pending:
        return !task.isDone;
      case _IsarTaskFilter.completed:
        return task.isDone;
      case _IsarTaskFilter.starred:
...
class _IsarHeroCard extends StatelessWidget {
  const _IsarHeroCard({
...
class _IsarTaskCard extends StatelessWidget {
  const _IsarTaskCard({
    required this.task,
    required this.accentColor,
