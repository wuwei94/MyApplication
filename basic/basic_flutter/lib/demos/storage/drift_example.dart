...
import 'package:basic_flutter/demos/storage/drift/drift_task_database.dart';
import 'package:flutter/material.dart';
import 'package:timeago/timeago.dart' as timeago;

/// Drift
/// https://pub.dev/packages/drift
class DriftDemoPage extends StatelessWidget {
  const DriftDemoPage({super.key, required this.title});

  final String title;

  @override
  Widget build(BuildContext context) {
    return DriftDemoView(title: title);
  }
}

class DriftDemoView extends StatefulWidget {
  const DriftDemoView({super.key, required this.title});
...
class _DriftDemoViewState extends State<DriftDemoView> {
  ...
  late final Future<DriftTaskDatabase> _databaseFuture;

  _DriftTaskFilter _selectedFilter = _DriftTaskFilter.all;
  int _selectedPriority = 2;
  bool _isMutating = false;
  String _statusMessage = '正在准备 Drift Database，稍后会自动加载演示数据。';
  ...
  void initState() {
    ...
    _databaseFuture = _prepareDatabase();
  }
  ...
  Future<DriftTaskDatabase> _prepareDatabase() async {
    final DriftTaskDatabase database = await DriftTaskDatabase.create();
    if (!mounted) {
      return database;
    }

    setState(() {
      _statusMessage = 'Drift 已就绪，当前示例通过 watch() 自动监听 SQL 查询结果。';
    });
    return database;
  }
  ...
  Future<void> _runMutation({
    ...
    required Future<void> Function(DriftTaskDatabase database) mutation,
  }) async {
    if (_isMutating) {
      return;
    }
    ...
    try {
      final DriftTaskDatabase database = await _databaseFuture;
      ...
      setState(() {
        _statusMessage = 'Drift 操作失败：$error';
      });
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _createTask() async {
    ...
    if (title.isEmpty) {
      setState(() {
        _statusMessage = '先输入一个标题，再把记录写入 Drift。';
      });
      return;
    }

    await _runMutation(
      successMessage: '已创建一条新任务记录，查询结果会立即自动刷新。',
      mutation: (DriftTaskDatabase database) {
        return database.addTask(
          title: title,
          detail: detail.isEmpty ? '这是一条刚写入 Drift 的示例记录。' : detail,
  ...
  Future<void> _resetDemoData() async {
    await _runMutation(
      ...
      mutation: (DriftTaskDatabase database) {
        return database.resetWithDemoData();
      },
    );
  }
  ...
  Future<void> _clearAllTasks() async {
    await _runMutation(
      successMessage: 'Drift 中的演示记录已清空。',
      mutation: (DriftTaskDatabase database) {
  ...
  Future<void> _toggleDone(DriftTask task) async {
    final bool nextState = !task.isDone;
    await _runMutation(
      successMessage: nextState ? '任务已标记完成。' : '任务已恢复为待处理。',
      mutation: (DriftTaskDatabase database) {
        return database.toggleDone(task);
      },
    );
  }

  Future<void> _toggleStarred(DriftTask task) async {
    final bool nextState = !task.isStarred;
    await _runMutation(
      successMessage: nextState ? '这条任务已加入星标。' : '这条任务已取消星标。',
      mutation: (DriftTaskDatabase database) {
        return database.toggleStarred(task);
      },
    );
  }
  ...
  Future<void> _deleteTask(DriftTask task) async {
    await _runMutation(
      successMessage: '已删除一条本地记录。',
      mutation: (DriftTaskDatabase database) {
        return database.removeTask(task.id);
      },
    );
  }

  List<DriftTask> _applyFilter(List<DriftTask> tasks) {
    return tasks
        .where((DriftTask task) => _selectedFilter.matches(task))
        .toList();
  }
  ...
  Widget _buildComposerSection() {
    return _SectionCard(
      title: '记录写入',
      subtitle: '输入标题和描述后直接写入 Drift，下面的列表会通过 watch() 自动刷新。',
      child: Column(
          ...
          Wrap(
            ...
            children: <Widget>[
              FilledButton.icon(
                ...
                label: Text(_isMutating ? '处理中...' : '写入记录'),
  ...
  Widget _buildFilterSection({
    ...
    return _SectionCard(
      ...
      child: Wrap(
        spacing: 8,
        runSpacing: 8,
        children: _DriftTaskFilter.values
            .map(
              (_DriftTaskFilter filter) => ChoiceChip(
  ...
  Widget _buildTaskListSection(List<DriftTask> tasks) {
    if (tasks.isEmpty) {
      return _SectionCard(
        title: '记录列表',
        subtitle: '当前筛选条件下还没有记录，可以先写入一条数据或恢复演示数据。',
        child: _buildCenteredState(
          icon: Icons.inbox_outlined,
          title: '列表为空',
          description: 'Drift Database 已准备好，现在可以体验新增、更新和删除操作。',
        ),
      );
    }
    ...
    return _SectionCard(
      title: '记录列表',
      subtitle: '每一张卡片都对应一条本地记录，支持更新状态、星标和删除。',
      child: Column(
        children: tasks
            .map(
              (DriftTask task) => Padding(
                padding: const EdgeInsets.only(bottom: 12),
                child: _DriftTaskCard(
                  task: task,
                  accentColor: _accentColor,
...
enum _DriftTaskFilter {
  all('全部'),
  pending('待处理'),
  completed('已完成'),
  starred('星标');

  const _DriftTaskFilter(this.label);

  final String label;

  bool matches(DriftTask task) {
    switch (this) {
      case _DriftTaskFilter.all:
        return true;
      case _DriftTaskFilter.pending:
        return !task.isDone;
      case _DriftTaskFilter.completed:
        return task.isDone;
      case _DriftTaskFilter.starred:
...
class _DriftHeroCard extends StatelessWidget {
  const _DriftHeroCard({
...
class _DriftTaskCard extends StatelessWidget {
  const _DriftTaskCard({
    required this.task,
    required this.accentColor,
