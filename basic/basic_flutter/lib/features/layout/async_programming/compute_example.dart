...
class ComputeExample extends StatelessWidget {
  const ComputeExample({super.key, required this.title});

  final String title;

  @override
  Widget build(BuildContext context) {
    return ComputeRoute(title: title);
  }
}

class ComputeRoute extends StatefulWidget {
  const ComputeRoute({super.key, required this.title});

  final String title;

  @override
  State<ComputeRoute> createState() => _ComputeRouteState();
}

class _ComputeRouteState extends State<ComputeRoute> {
  int _result = 0;
