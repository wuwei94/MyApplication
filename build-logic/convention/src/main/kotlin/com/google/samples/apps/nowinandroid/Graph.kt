...
/**
 * 遍历映射集合并过滤 null 返回的工具函数
 */
private inline fun <T, R : Any> Iterable<T>.associateWithNotNull(transform: (T) -> R?): Map<T, R> {
    val result = mutableMapOf<T, R>()
    for (element in this) {
        val transformed = transform(element)
        if (transformed != null) {
            result[element] = transformed
        }
    }
    return result
}
...
private abstract class GraphDumpTask : DefaultTask() {
    ...
    private fun Pair<String, String>.toDependency(project: String) = Dependency(project, configuration = first, dependency = second)
