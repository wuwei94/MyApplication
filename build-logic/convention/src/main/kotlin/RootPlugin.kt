...
abstract class RootPlugin : Plugin<Project> {
    ...
    override fun apply(target: Project) {
        ...
        if (!buildFeatures.isIsolatedProjectsEnabled()) {
            ...
            target.tasks.register("generateModulesGraph") {
                group = "documentation"
                description = "Generates and updates module dependency graphs in all README.md files"
                dependsOn(target.subprojects.mapNotNull { it.tasks.findByName("graphUpdate") })
            }
        }
...
private fun BuildFeatures.isIsolatedProjectsEnabled(): Boolean = isolatedProjects.active.getOrElse(false)
